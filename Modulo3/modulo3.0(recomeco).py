...
# objetos também podem ser chamados de instâncias. Você pode chamar qualquer
# coisa que foi gerada por uma classe de instância ou objeto
...
# para criar uma classe própria, chamamos a indicativa class e depois inserimos
# o nome da classe
class Pessoa:
...
p1 = Pessoa() #-> isso aqui é uma instância/objeto e é através dela que
              #   vou nomear atributos
p1.nome = 'Maria' # atribui um atributo,denominado nome
p1.idade = 19 # atribui um outro atributo,denominado idade

# se eu printar assim, vai mostrar apenas o espaço na memória, pois os
# atributos estão contidos/protegidos na classe
print(p1)
# logo, para chamar do jeito correto, basta:
print(p1.nome) # para acessar o nome
print(p1.idade) # para acessar a idade

#### É possível criar uma outra instância/objeto, sendo para p2
p2 = Pessoa()
p2.nome = 'Clara' 
p2.idade = 22

#printando nome e idade, que nesse caso, como houve uma nova instância/objeto,
# serão diferentes dos primeiros.
print(p2.nome, p2.idade)

####----
# No entanto, fazer dessa forma seria bem chato. Ter que definir tudo isso na
# mão parece ser bem tediante. É por isso que foi criado o self, que nada mais
# é que uma instância temporária.
# Imagine a seguinte definição: self = class() -> logo, self.nome = 'João'
# Dito isso, deixa eu mostrar na prática:

class Alunos:
    # para isso funcionar, vamos precisar de um inicializador/construtor, que
    # é o parâmetro init.
    # ele é responsável por iniciar a nossa instância e atributos de classe.
    def __init__(self, nome, turma): # e aqui, passamos o self
                                     # (lembre-se do exemplo anterior).
        # acima, vou definir os parâmetros que eu quero
        # (já que se trata de uma função)
...
aluno1 = Alunos('Carlos', '2° Ano')# e no parênteses, eu passo os argumentos
                                   # (como uma função mesmo).
...
###############################################################################
#Aula 200 e 201
# Um método é uma função que se encontra dentro da classe, normalmente tendo
# um 'self' como primeiro argumento, com o intuito de referenciar instâncias.
# Ex:

class Carro(): #aqui é a classe
    def __init__(self, nome_do_carro): # este é o construtor + nome do veículo
        self.nome = nome_do_carro # quando eu denominar um objeto e chamar essa
                                  # classe, o argumento que ele passar virá
                                  # para cá.
    
    # é sabido que um carro acelera e freia, né? para fazer com que isso 
    # ocorra, vamos precisar de um método, já que um método ocasiona em uma
    # ação e não consiste em exibir strings pré-definidas nos argumentos
    # da classe, como um atributo

    # isso aqui é um método (uma função dentro da classe) e se realmente
    # houvesse um carro correndo, bastaria mexer na variável que guarda sua
    # velocidade em determinada situação.
    def acelerar(self):
        print(f'O {self.nome} acelerou e está a 100km/h')

fusca = Carro('Fusquinha')
print(fusca.nome) # chamando o atributo nome, que definimos na chamda da classe
fusca.acelerar() # e aqui o método, que fez com que o carro andasse mais rápido.

'''
Em resumo, o self é utilizado como referência para a instância. É de fato como
se fosse uma variável que será substituída pelo nome do objeto que você
declarar fora da classe.

Método é uma função dentro da classe, que ocasiona em ações. Ex: Upper ou
lower da class str

atributo são valores pré-definidos que são inseridos na criação de um objeto.

curiosidade: o parâmetro self foi definido assim por conveção entre os
programadores. Ou seja, se você quisesse colocar qualquer outro nome no lugar
de self como primeiro parâmetro e não usasse ele, funcionaria.
'''
...
try:
    ...
    print(e) # e como ela necessita da instância, vai dar pau se chamamos
             # ela dessa forma.

# no entanto, poderiamos passar esse argumento que ela necessita.
Carro.acelerar(fusca) # e assim, ela não dará pau, pois agora ela tem a
                      # instância como argumento.
# Segundo o Luiz Otávio, esse não é um jeito comum de uso, mas é bom para
# entendimento e demonstração de seu funcionamento

###########################################################################################
# Aula 202, 203, 204 e 205
# Diferença entre atributo de classe e atributo de instância
class Animal:
    # abaixo, defini um atributo de classe, que não precisa necessariamente de
    # uma instância para funcionar.
    # Além disso, o atributo de classe pode ser utilizado em qualquer lugar
    # da classe, ou seja, método.
    area = 'Terrestre' #Aqui será utilizado apenas para animais terrestres.

    def __init__(self, nome):
        # aqui, definiremos um atributo de instância, ou seja, que é protegido
        # pelo namespace (escopo da função) o atributo de isntância poderá ser
        # utilizado apenas quando a função que o guarda ser acessada. 
        self.nome = nome

        # para acessar um atributo de classe, precisamos primeiro acessar o
        # namespace da classe.
        # Para isso, podemos fazer de duas formas: Através do próprio self ou
        # do nome da classe em si. Ex:
        print(self.area) #acessando pelo self
        print(Animal.area) #acessando diretamente pela classe.
        # segundo o luiz otávio, é mais recomendado que você faça o acesso
        # diretamente pela classe para evitar transtornos no futuro.
        # Portanto, realmente é melhor fazer uso através do escopo da classe,
        # pois se houver um atributo de instância e um atributo de classe com os mesmos nomes, um iria sobreescrever o outro.
        

    #Agora, vamos decidir um método, ou seja, uma ação.     
    def comer(self, alimento):
        # realizando ação e ainda assim, utilizando atributo de classe
        # e de instância
...
# e para pura demonstação e diferentemente de um atributo de instância,
# eu consigo acessar tranquilamente um atributo de classe estando fora
# da classe e sem necessitar de objeto algum.
print(Animal.area)
...
## Em poo, as classes tem estado, como high ou low em C+. Ou seja, podemos
## habilitar uma série de ações com base nela. Ex:
class Camera:
    def __init__(self, nome, estado_filmando=False):# como eu liguei a camera
    ...
    def filmar(self):
        # mas antes de apertar no botão de filmar, precisamos verificar se ela
        # não está filmando
        if self.filmando:
...
###################################################################
# Aula 206, 207 e 208
#__dict__ e vars -> fazem a mesma coisa com uma leve diferença, onde um é
# método e o outro é uma função.
# O objetivo deles é acessar um atributo de instância e retornar um dicionário,
# contendo como chave o nome da instância e como valor o seu atributo.
# Ex:
...
# e um detalhe importante é que isso não é apenas leitura. Ou seja, através
# de um deles posso alterar, incrementar ou remover valores, como um dicionário
# mesmo. ex:
...
# e para deixar claro, esse tipo de manipulação não é comum. Mas mesmo assim,
# é bom saber pra saber que existe!


#### Aula 209
# Como você viu na aula anterior, usamos certas convenções para nomes de
# variáveis, funções, classes e assim por diante.
# Essas convenções tem um nome que podemos usar para nos referir ao modo como
# estamos nomeando determinados objetos em nosso programa: PascalCase,
# camelCase e snake_case.

# PascalCase - significa que todas as palavras iniciam com letra maiúscula e
# nada é usado para separá-las, como em: MinhaClasse, Classe, MeuObjeto,
# MeuProgramaMuitoLegal. Essa á a convenção utilizada para classes em Python;

# camelCase - a única diferença de camelCase para PascalCase é a primeira 
# letra. Em camelCase a primeira letra sempre será minúscula e o restante das
# palavras deverá iniciar com letra maiúscula. Como em: minhaFuncao,
# funcaoDeSoma, etc... Essa
# conversão não é usada em Python (apesar de eu confundir as duas e às vezes
# acabar chamando camelCase de PascalCase ou vice-versa, mas agora você sabe
# a diferença);
...
# Os padrões usados em Python são: snake_case para qualquer coisa e PascalCase
# para classes.

##########################################################################
# Aula 210 - classmethods + factories methods
class Pessoa2:
    ano_atual = 2025 # -> lembrando que isso aqui é um atributo de classe,
                     #    ou seja, eu consido acessar fora da classe sem 
                     #    necessitar definir uma instância

    # abaixo, estará o molde do objeto, necessitando de instância e de
    # dois atributos.
    def __init__(self, nome, idade):
        self.nome = nome 
        self.idade = idade
    
    # ou seja, acessar atributos, temos o dois meios, sendo um através da
    # classe (molde) e outro através de uma instância.

    # Para métodos, temos a mesma lógica. Exemplo, vamos criar um método
    # abaixo:
    def exibir_nome(self):
        return f'seu nome é {self.nome}'
    
    # mas... você pode até perguntar: todo método precisa necessariamente de
    # uma instância? NÃO! Basicamente, temos um decorador denominado de
    # classmethod (método de classe), que é bem semelhante a um  atributo de
    # classe. Ou seja, você poderá executar o método sem necessitar da 
    # instância, acessando somente pelo namespace da classe.

    ## Classmethods
    @classmethod
    def exibir_ola(cls): # enquanto self faz referência a instância, cls faz
        #  referência a própria classe (molde). 
        # Pra você entender bem, imagine a seguinte atribuição:
        # -> cls = Pessoa2 <- Considernando que o nome da classe é pessoa2
        # importante: O classmethod por si só não consegue acessar atributos
        # de instância self, por isso, pode ser executado diretamente sem
        # necessitar de um objeto em questão.
        return 'ola'
    
    ## Factory Method
    # Usa @classmethod, logo recebe cls (classe).
    # Enquanto classmethod acessa a classe, o factory method cria uma nova
    # instância da classe com parâmetros diferentes dos passados diretamente
    # ao construtor.
    @classmethod
    def criar_sem_nome(cls, idade):
        # cls aqui funciona como um callable, chamando __init__ com os
        # argumentos definidos
        return cls("Anônimo", idade)

print(Pessoa2.ano_atual) # acessando o atributo de classe sem instância
EP1 = Pessoa2('Pedro', 19) # definindno a instância e os atributos necessários
                           # para a classe
print(EP1.exibir_nome()) # aqui, conseguimos executar o método em questão 
                         # devido a existência da instância.
print(Pessoa2.exibir_ola()) # demonstrando o uso do classmethod, sem prcisar
                            # necessariamente da instância para execução.
...
class Pessoa3:
    ...
    def from_dict(cls, dados):
        # Factory method que recebe um dicionário e trata para inicializar
        # a classe
...
# Ou seja, esse factories methods basicamente fez o tratamento dos dados 
# recebidos antes de entrar diretamente na classe. Com isso, não precisamos
# tratar diretamente no init, que sobrecarregaria um pouco a classe, 
# principalmente em casos de dados que não precisem ser tratados. Ademais,
# posso fazer inúmeros factories methods dentro de uma classe, ou seja,
# inúmeros tratamentos à parte.
...
class Connection:
    ...
    def __init__(self, host='localhost'): # em redes, o padrão para host
                                          # é localhost
        self.host = host
        self.user = None # Como não é sabido o usuário e a senha, serão None
        self.password = None
    
    #configurando um setter para "usuário"
    def set_user(self, user): # setter vai servir como um meio para alterar de
                                # None para outra coisa
        self.user = user
    ...
    ###
    @classmethod # denomino uma classe que vai passar diretamente os argumentos
                 # para init
    def create_with_auth(cls, user, password): # passo os parâmetros
        connection = cls() # inicializo o init 
        # após inicializar o init, conseguirei ter acesso aos atributos
        # de instância. Com isso, basta apenas realizar as modificações
    ...
    ### O staticmethod, como mencionado anteriormente, será utilizado como uma
    ###  função normal. Pois não acessa cls e nem self.
    @staticmethod
    def soma(x, y):
        return x+y
...
# Nesse caso, não precisaremos passar os argumentos da classe diretamente 
# pelo objeto
...
# getter - um método para obter um atributo, ou seja, você utiliza de um método
# \para que lhe seja retornado um atributo.
# Isso pode ser útil em um caso de código cliente, onde um ou mais 
# desenvolvedores fazem uso de sua classe para sustentação de um código.
# Com isso, é bem provável que haja mudanças em suas classes e isso pode gerar
# transtornos no código cliente.
# Por exemplo: Se hoje você define o nome de uma instância de uma forma e
# amanhã quer trocar esse nome? essa ação não iria ocasionar em transtornos no
# código cliente? é a mesma coisa que você declarar uma variável, utilizar
# essa variável em outro lugar e posteriormente alterar o nome da variável em
# sua definição. Com isso, surge um problema... como eu poderia resolver essa
# questão? Uma das formas seria utilizar o getter, que protegeria o escopo
# dentro de uma função. EX:

class Caneta:
    def __init__(self, cor, marca):
        # você poderia instanciar assim e falar que já está pronto para uso.
        # Porém, caso você mudasse o nome do atributo "cor", geraria
        # transtornos.

        self.cor = cor # por isso, é possível instanciar e deixar um método
                       # fazer o restante
    ...
    def get_cor(self):
        # toda que precisar deste atributo fora da classe, será chamado esse
        # "getter" genérico. Logo, problema seria resolvido.
        return self.cor
    
    ### @property - um getter no modo Pythônico
    # O getter acima iria funcionar perfeitamente na maioria das linguagens.
    # No entanto, em python, temos uma forma diferente de fazer isso que 
    # funciona melhor que o jeito anterior, sendo ele o property.
    # Property basicamente é um decorator que quando inserido numa funcão, 
    # executa esta função como se ela fosse um atributo qualquer,
    # sem necessitar dos "()", como se a funcão não fosse mais um executável (
    # callable )

    @property # incrementando o decorator
    def marca(self): 
        return self.marca_caneta # aqui será apontado para um atributo
                                 # dentro da classe. Logo, quando caneta.marca
                                 # for chamada fora da classe, esse método
                                 # será executado e como consequência, irá
                                 # retornar o nome da marca da caneta.
...
print(caneta.get_cor()) # isso iria garantir uma proteção extra ao 
                        # código cliente.
...
class Filme:
    def __init__(self, nome, categoria):
        # definindo um atributo de instância que pode ser acessado de 
        # qualquer forma
        self.nome = nome

        # nesse aqui, vamos fazer uma property
        self._exibir_categoria = categoria # Em poo, temos algo chamado de
                                           # "protected", que informa por
                                           # convenção que atributos ou 
                                           # métodos que começa com underline
                                           # não deverá ser acessado fora da
                                           # classe. Por isso o "_".

    # realizando o getter, como na classe anterior.
    # (o getter vai retornar o conteúdo presente no atributo de instância
    # _exibir_categoria)
    @property
    def categoria(self): 
        return self._exibir_categoria
    
    # realizando um setter: 
    # O setter é equivalente a uma mudança no atributo de instância
    # _exibir_categoria. Ou seja, se o usuário resolver alterar o nome do
    # atributo, é necessário fazer a configuração a seguir para não ocasionar
    # transtornos.

    # Para isso, vamos precisar utilizar um decorator que apontará para a
    # função property e especificar o setter, veja abaixo:
    @categoria.setter 
    # agora precisaremos repetir a mesma função da pŕoperty.
    # (Não se preocupe, ninguém vai sobrescrever ninguém.)
    def categoria(self, valor): # -> como parâmetro, é necessário especificar
                                #    o self e posteriormente, o valor que será
                                #    passado como argumento.

        if 2 == 2: # logo, é possível realizar duas verificacões 
            self._exibir_categoria = valor # -> o "valor" que for passado
                                           #    será inserido no atributo de
                                           #    instância "_exibir_categoria"
        else:
...
#No caso da property
f1.categoria = 'Aventura' # você nem percebe que isso aqui é uma propery e que
                          # tem uma estrutura complexa rodando por baixo.
...
class Cara:
    ...
    @nome.setter
    def nome(self, valor):
        print('estou no setter') # Chamo inicializar a classe, no mesmo
                                 # momento, isso será imprimido.
        self._nome = valor
...
#       "name mangling" (desfiguração de nomes) em Python. Após sofrer isso,
#       o objeto fica com o aspecto semelhante a 
#       _NomeClasse__nome_attr_ou_method. Sendo que só DEVE ser usado na classe
...
class Persona:
    ...
    def _exibir_idade(self):
        # acessando protected corretamente (dentro da classe que fora definido,
        # também podendo ser acessado em uma herança)
        return self._idade
    # método private
...
# aviso: vai funcionar perfeitamente. Mas, saiba que estará indo 
# contra a conversão.

## Demonstrando o acesso a modificadores de acesso private
...
try:
    ...
    #detalhe -> isso foi apenas para demonstração. Em hipótese alguma você
    # deverá acessar um atributo ou método privado fora da classe ou em uma
    # subclasse. Isso só deverá ser acessado dentro da classe. Se fizer o
    # contrário, vai funcionar, mas estará indo contra a convensão.

cls()
###############################################################################
### Relações entre classes: Associação, Agregação, e composição.

# Associação é uma relação fraca entre duas classes, que liga uma a outra
# no mesmo sistema.
#
# Relação fraca -> Não há dependência. Ou seja, você pode utilizar as classes
# em conjunto ou individualmente.
#  
# Na natureza, associação seria algo semelhante a uma protocooperação entre
# objetos. Ou seja, eles não precisam um do outro para realizar suas tarefas,
# mas junto com o outro, pode funcionar melhor.
#
# Além disso, quando realizamos uma protocooperação entre objetos, podemos
# utilizar seus atributos ou métodos estando em outra classe 
# (é por isso que o método escrever funciona sem problemas na classe escritor).

#definindo a primeira classe
class Escritor:
    def __init__(self, nome):
        self.nome = nome
        # aqui vai ficar a outra classe ou atributo mesmo. Como não há
        # nada ainda, será None.
    ...
    # o setter para atribuir o valor para ferramenta, podendo ser uma
    # classe ou valor qualquer.
    @ferramenta.setter
...
#lembra do atributo de instância ferramenta, que tem setter? é lá que vamos
# colocar o objeto 
escritor.ferramenta = caneta # os métodos e atributos que estão em canetas
                             # poderão ser acessados através do objeto escritor
#método escrever
print(escritor.ferramenta.escrever())
#atributo de instância nome
print(escritor.ferramenta.nome)

# Por fim, foi provado que na associação a protocooperação de fato é efetiva. 
...
#
# No exemplo abaixo, vamos utilizar o objeto carrinho de compras e produtos,
# no contexto de um mercado. 
# Para um melhor contexto, é sabido que você pode fazer uso de um carrinho
# sem colocar produtos lá dentro, correto? Assim como também é possível pegar
# um produto sem necessariamente precisar 
# de um carrinho. No entanto, quando se trata de vários produtos, para maior
# agilidade de coleta e somatória, é melhor pegar o carrinho... assim, será
# muito mais rápido tanto para você como para o caixa.
...
class Carrinho:
    ...
    # uma funcionalidade extra e útil para o carrinho seria também a somatória
    # dos produtos (o que não é necessário, mas é útil)
    def somar_tudo(self):
         print(sum(
            produto.preco
...
# Logo, foi provado que o objeto p1 (o mesmo vale para p2) não
# precisa do carrinho.
...
# Com isso, também foi provado que o carrinho não precisa do produto para
# funcionar. Ou seja, ambas funcionam (o que prova a relação fraca),
# mas na maioria dos casos, usar dessa forma não vai ser muito eficiente...
# por exemplo, e se for mais de um produto e ambos fossem maiores?
# haverá problemas.

# Por isso, podemos combinar da seguinte forma:
c.inserir_produtos(p1, p2) #colocamos os objetos que guardam os produtos no
# método inserir_produtos, da classe Carrinho.
# agora, eu consigo ver quais produtos tem no meu carrinho
c.listar_produtos()
# e também conseguimos somar tudo
c.somar_tudo() 

# Com isso, aprendemos um pouco do funcionamento da agregação, que é um tipo
# específico de associação entre classes.
...
class Livro:
    def __init__(self, titulo, total_paginas):
        ...
        # Perceba também que a classe página foi inicializada dentro da classe
        # livro, criando assim uma relação entre classes.
    ...
    # Em Python, __del__ (destrutor) é executado quando o garbage collector
    # decide liberar um objeto que não tem mais referências. O comando del
    # apenas remove uma referência, o que pode levar o GC a destruir o objeto
    # e disparar __del__.
    def __del__(self):
...
class Pagina:
    ...
    # Quando o objeto que inicializa a classe livro for apagada, a classe
    # página será também será apagada, já que a vida da classe Página será
    # gerenciada pela classe Livro quando Livro for inicializada em um objeto.
    def __del__(self):
...
# Herança não faz parte da composição, pois não se trata de uma relação de 
# "ter", e sim de "ser".
...
# Refere-se à ordem de busca de métodos e atributos em uma hierarquia de 
# classes.
# No exemplo, Usuario herda de Pessoa, e ambas herdam de object
# (implícito em Python 3).
...
# Ao usar herança, você herda atributos e métodos da classe mãe.
# Isso significa que parte do comportamento já vem pronto e, em muitos casos,
# não é necessário reescrevê-lo.
#
# O problema é que, quanto mais níveis de herança você cria, mais difícil
# fica entender de onde vem cada método ou atributo. Esse acoplamento pode
# deixar o código complexo difícil de manter.
#
# Por isso, uma boa prática para iniciantes é limitar a hierarquia a, no
# máximo, 3 níveis. Caso acabe passando disso, geralmente é mais indicado usar
# composição (relação entre classes) em vez
# de herança profunda.

cls()
### @___
### Eu também posso herdar classes que já existem e fazer alguma mudança.
### Por exemplo:
...
# Os demais métodos que não foram modificados da classe str, vão
...
### Uso do super
# no caso de você querer fazer uma modificação leve em um método, como
# um logger:
class MinhaClasse2(MinhaClasse): #-> vou herdar os atributos e métodos da
                                 # minha classe
    def lower(self):
        print('Vou deixar tudo minúsculo!')
        # o super busca na superclasse o método existente. Com isso, podemos
        # utilizar o super como uma ponte para retornar o próprio método com
        # o estado anterior.
        return super().lower() 
        ...
        #
        # Mas, em python, quando você chama o super da forma como eu fiz no
        # return, ele já vem com os parâmetros declarados iniciamente dessa 
        # forma como demontrei acima, evitando verbosidade.
        #
        # No entanto, sabemos que o super dessa classe é "MinhaClasse". Isso
        # acontece porque, ao chamar super(), o Python entende automaticamente
        # que estamos dentro da classe "MinhaClasse2", que, por sua vez, é uma
        # subclasse de "MinhaClasse".
        #
        # Assim, o interpretador reconhece que "MinhaClasse2" possui uma
        # superclasse e faz a busca nela. É justamente esse processo que
        # permite que o método "lower()" seja executado, mesmo não existindo
        # diretamente no escopo de "MinhaClasse2". O Python então localiza o
        # método correspondente na classe mãe ("MinhaClasse") e o executa.
...
class Animal:
    def __init__(self, especie): # o constutor necessitou de um parâmetro,
                                 # que no caso, foi a espécie
        self.especie = especie

# como as classes abaixo são classes filhas, ao serem inicializadas, irão
# precisar do argumento "espécie" também para inicializar.
class Gato(Animal):
    ...
    #
    # Para evitar isso, usamos o “super()” para acessar o construtor da classe
    # mãe (Animal) e reaproveitar seus parâmetros.

    def __init__(self, especie, raca): # eu sei que o parâmetro que tem lá é
                                       # especie, logo, adiciono ele no
                                       # construtor.
        self.raca = raca
        super().__init__(especie) # e para acessar, utilizo o super para fazer
                                  # o acesso ao parâmetro original
        # Com isso, eu crio um novo parâmetro no construtor e já inicializo 
        # com o self, presente na classe original. 

    # Além disso, consigo incrementar um método aqui também, que seja apenas
    # dessa classe e de suas filhas.
    def ronronar(self):
        print('vrum vrum cvrummm')

# Mas, quando os parâmetros de uma superclasse são inúmeros, passar cada
# parâmetro do init em um super acaba sendo um tanto quanto demorado...
# é por isso que há uma segunda opção: Utilizar argumentos nomeados e não 
# nomeados com empacotamento e desempacotamento. Veja no exemplo abaixo:
class Cachorro(Animal):
    # Inicialmente, chamo o método que precisará ser modificado e passo o 
    # parâmetro que quero incrementar.
    # Após passar tal parâmetro, passo *args e **kwargs para empacotar todos
    # os parâmetros recebidos, de forma flexível
    def __init__(self, sexo,*args, **kwargs):
        # agora eu posso instanciar o novo parâmetro
        self.sexo = sexo
        # e no super, chamar o init da superclasse e desempacotar os parâmetros
        # lá existentes.
        super().__init__(*args, **kwargs)

        # Com isso, modifiquei o construtor sem necessitar adicionar os 
        # parâmetros manualmente.

        # Desvantagem: Não preserva a tipagem e na chamada do método para
        # instanciar externamente (criação de um objeto à partir da classe),
        # os parâmetros não aparecem no namespace para autocomplete.
...
# Inicialmente, vamos criar três classes que receberão os mesmos atributos
# de classe e métodos.
...
# Porém, no mro, a hierarquia começa da classe que foi chamada. No exemplo
# acima, se eu busco o mro à partir da classe A, o método "demonstrar"
# vai imprimir na tela a letra 'A'
#
# Isso acontece porque a busca no mro vai começar da classe ou opbjeto que ele
# foi chamado. Com isso, ele busca determinado método ou atributo na classe que
# eu chamei. Se ele não encontrar, ele busca na classe mãe (se houver).
# Se ainda assim na classe mãe não houver, ele busca nas outras classes que 
# estão ligados a ele através da Herança. E se ele não encontrar em nenhuma
# das subclasses, será retornado erro na tela. No entanto, se ele encontrar,
# vai parar de buscar e inicializar o método em questão. É como se fosse um
# for buscacando algo com auxilio de um if, que quando encontra,
# aciona o break.  

#exemplo:
# a busca começa na classe C, mas na classe C não existe atribuito de classe
# a,b ou c. Logo, ele buscará nas subclasses.
print(c.atributo_a) # inicializa em C, depois busca em B e logo busca 
                    # em A, encontrado. 
print(c.atributo_b) # inicializa em C, depois em B, encontrado.
print(c.atributo_c) # inicializa em C e encontra em C.

# abaixo, o demonstrar existe em todas. Tanto A, como B e C. Mas como eu disse,
# o mro começa a da classe que foi chamada e como em C existe o método
# demonstrar, a busca é finalizada. (famosa sobreposição)
c.demonstrar() # inicializa em C e encontra em C.

######
# Como foi visto anteriormente, em um dos casos de uso do super, podemos fazer
# alguma modificação em uma classe e ainda chamar o método da função 
# anterior, correto?
...
class D(C):
    ...
    def demonstrar(self):
        # super().demonstrar()
        # se eu deixar assim, ele vai chamar automaticamente o método da função
        # mãe de D, que no caso, é C. Logo, isso seria o equivalente a
        # super(D, self).demonstrar()
        #
        # Portanto, significa que eu indico que a classe é D (a atual) e também
        # indico o objeto (self). Mas e se eu falar pra ele que essa classe na
        # verdade é B?
        super(B, self).demonstrar() # retorno: A

        # Isso significa que eu disse pra ele que a classe que ele começará a
        # busca para o mro é B e como se trata de superclasse, ele foi procurar
        # a superclasse de B, que no caso é A.
        #
        # É como se eu falasse: Ei, agora você não deve iniciar a busca por D,
        # você deve começar por B.

        # Importante:
        # Se eu não quisesse usar o super() para acessar um método da classe
        # mãe, eu poderia chamar diretamente a classe base (caso eu saiba qual
        # é) junto com o método e o argumento de instância (self). Isso executa
        # algo semelhante ao super(), mas não faz a mesma coisa internamente,
        # ele apenas chama o método definido naquela classe específica,
        # ignorando a hierarquia de herança.
        #
        # Portanto, embora funcione, o mais recomendável é utilizar super(),
        # pois ele segue a MRO (Method Resolution Order) e garante que a busca
        # do método seja feita de forma correta e dinâmica, especialmente em
        # casos de herança múltipla.
        A.demonstrar(self)
...
# Em Python, assim como em algumas outras linguagens de programação, é
# possível utilizar o conceito de Herança Múltipla.
# Nos exemplos anteriores, quando criamos uma classe filha, passamos apenas
# uma classe entre parênteses, isso indica que essa classe é a superclasse
# (ou classe mãe).
# No entanto, também é possível adicionar duas, três ou mais classes entre
# parênteses, separadas por vírgulas. Quando isso ocorre, dizemos que estamos
# utilizando Herança Múltipla, pois a nova classe passará a herdar atributos
# de classe, métodos e construtores de todas as classes que foram declaradas
# na herança.
#
# Contudo, isso torna o MRO (Method Resolution Order) um pouco mais complexo,
# já que o Python precisará determinar a ordem exata em que as classes serão
# pesquisadas quando houver métodos ou atributos com o mesmo nome.
# Ainda assim, o MRO é inteligente o suficiente para resolver esses conflitos
# de forma consistente, o verdadeiro desafio está em manter o código
# compreensível e organizado à medida que a hierarquia cresce.
#
# Além disso, hás casos que será utilizado mixin, que nada mais é que uma
# classe usada em herança múltipla para adicionar funcionalidades específicas
# a outras classes, sem fazer parte direta da hierarquia principal
# dessas classes.
# Ela é pensada para ser combinada (mixada) com outras classes, e não para ser
# instanciada por conta própria.
...
class Ovelha(logg, Animal): # foram passadas as duas classes anteriores
    def __init__(self, nome, especie):
        self.nome = nome
        # e aqui, vou acessar a classe mãe que possui o parâmetro especie
        # no init
    ...
    # resultado: Utilizei funcionalidades de duas classes diferentes em apenas
    # uma classe.
...
# Observação:
# No mro acima, percebemos que após ele ter buscado na própria classe, ele
# buscou em logg, para depois buscar em animal e por fim, object builting.
# No entanto, nem sempre isso acontece... pode haver casos que por mais que 
# determinada fosse passada primeiro, ela pode acabar não sendo a próxima a
# ser chamada no mro.
# 
# Isso acontece porque o python utiliza um algoritmo para saber qual classe
# deverá procurar depois, que é o C3 superclass Linearization. Esse algoritmo
# é o que o python utiliza atualmente, mas pode mudar.
# Nesse algoritmo, conforme a classe vai ficando complexa (com muitas heranças)
# mais difícil será prever o mro e poderá possibilitar o caso citado acima.
#
# Por isso existem o método .mro() e o atributo __mro__, que ajudam a 
# inspecionar e debugar a estrutura de herança. Ainda assim, lembre-se:
# segundo a PEP 8 e o Zen do Python, "A complexidade é inimiga da clareza." 

### Problema Diamante
# Como dito anteriormente, em herança múltipla, uma classe pode herdar 
# elementos de duas ou mais classes, correto? Dito isso, há um caso que possui
# um certo nível de complexidade, que é o caso do diamanate.
# Esse caso ocorre quando duas classes mãe tem uma outra classe mãe em comum
# na sua definição, possibilitando uma sobreposição errônea.
...
# presente em todas as quatro? o mro vai utilizar o seu algoritmo para 
# cálcular isso.
# Porém, como ainda se trata uma complexidade relativamente simples, ainda
# poderemos deduzir.

# sendo: o mro vai buscar em quatro e vai encontrar. Mas, caso não encontrasse,
# iria buscar em dois já que dois está na frente. Se não achasse em dois, iria
# buscar em tres e se não encontrar em três,
# buscaria em um.

exam = quatro() 
exam.quem_sou() # dois

# e como sabemos, "um" é a mãe de dois e três, que parece que vão aparecer mais
# de uma vez no mro, né? mas não. Quando o mro faz a busca, antes ele faz a
# mesclagem e assim, evita duplicações. Veja na saída abaixo:
...
#
# Em Python, isso também acontece com classes, onde podemos criar estruturas 
# que definem o comportamento esperado (abstração), mas deixam a implementação
# para as classes filhas. Veja:
...
class Notificacao(ABC):
    ...
    # enviar será um método abstrato, mas vamos definir que seu retorno será
    # bool.
    @abstractmethod
...
class Ponto:
    ...
    def __repr__(self):
        ...
        # retorno: <class '__main__.Ponto'>(x=50, y=60, z='string')
        return f'{nome_classe}(x={self.x}, y={self.y}, z={self.z})' 
        # retorno: <class '__main__.Ponto'>(x=50, y=60, z=string)
        #
...
# soma (+) em alguma parte do codigo, como 2 + 2. Quando os valores são 
# inicializados através de uma classe, o desenvolvedor pode alterar o 
# comportamento desses métodos.
